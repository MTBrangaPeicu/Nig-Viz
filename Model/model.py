...
def handle_pruned_forward_pass_request(doc):
    try:
        ...
        # NEW: Get pruning rules, targets, and edges from the frontend
        ...
        print(f"Pruning edges: {pruning_edges}")
        ...
        # Generate pruning masks using original NIG data PLUS rules, targets, and edges
...
def handle_conditional_nig_request(doc):
    ...
    if lock.locked():
        conditional_nig_service.patch(id, {"status": "pending"})
    lock.acquire()
    try:
        conditional_nig_service.patch(id, {"status": "processing", "progress": 0})
