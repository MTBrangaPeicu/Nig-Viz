...
class DataStore:
    def __init__(
        ...
        # self.connect_callback = lambda x: None
    ...
    def login_local(self, email, password):
        ...
        if "code" in res:
            ...
            print(f"Authenticated as: {res[1]['user']['email']}")
            self.access_token = res[1]["accessToken"]
        return self

    def login(self, email=None, password=None):
        if email is not None and password is not None:
            return self.login_local(email, password)
        if self.access_token is not None:
            return self.login_jwt()
...
class Service:
    def __init__(self, location, name, sio):
        ...
        (https://docs.feathersjs.com/), services provide a similar API with predefined
        CRUD methods.
