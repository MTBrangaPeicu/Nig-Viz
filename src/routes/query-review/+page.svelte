<script lang="ts">
  ...
  onMount(() => {
    ...
    if (passage2Container) passage2Unmount = passageInput2.mount(passage2Container);
    if (numRepsContainer) numRepsUnmount = numRepsInput.mount(numRepsContainer);
    if (subsetContainer) subsetUnmount = subsetButtonsComponent.mount(subsetContainer);

    // Subscribe to dataset service updates
    const datasetSub = nigConnection.datasetService.on('patched', (doc: any) => {
      if (doc.status === 'success' && doc.samples) {
        ...
        try {
          ...
          if (chosen) {
            ...
            const selectedSample = doc.samples.find((s: any) => s.query === chosen);
            if (selectedSample) {
    ...
    const querySub = queryInput.$value.subscribe((query: string) => {
      ...
      const selectedSample = queryInput.samples.find((s: any) => s.query === query);
      if (selectedSample) {
...
<div class="fixed inset-0 bg-base-200 flex flex-col">
  ...
  <div class="flex-shrink-0 bg-base-300 border-b border-base-content/10 px-4 py-2 flex items-center gap-4">
  ...
  <div class="flex-1 overflow-hidden flex">
    ...
    <div class="w-80 flex-shrink-0 bg-base-100 border-r border-base-300 overflow-y-auto">
    ...
    <div class="flex-1 overflow-auto bg-base-100">
      <div class="p-6">
        <div class="flex items-center gap-2 mb-4 border-b border-base-300 pb-2">
          <h2 class="text-sm font-semibold text-base-content/70">Token IG Results</h2>
        </div>
        
        {#if !igOutput}
          <div class="text-sm text-base-content/60">
            Submit a query and passage to see token-level integrated gradients visualization.
          </div>
